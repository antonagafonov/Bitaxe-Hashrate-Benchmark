...
def parse_arguments():
    ...
    parser.add_argument(
        "bitaxe_ip", nargs="?", help="IP address of the Bitaxe (e.g., 192.168.2.26)"
    )
    parser.add_argument(
        "-v",
        "--voltage",
...
def restart_system():
    try:
        ...
        if not is_interrupt:
            ...
            time.sleep(
                sleep_time
            )  # Allow sleep_time for the system to restart and start hashing
        else:
...
def reset_to_best_setting():
    if not results:
        ...
        print(
            GREEN + f"Applying the best settings from benchmarking:\n"
